...
voiceButton.states.weak =
	opacity: 0.3
	options: 
		time: 0.2
		curve: 'linear'

voiceButton.states.strong =
	opacity: 1
	options: 
		time: 0.2
		curve: 'linear'

voiceButton.stateSwitch('show')
	

voiceButtonText.states.weak = 
	opacity: 0.5
	options: 
		time: 0.2
		curve: 'linear'

voiceButtonText.states.normal = 
	opacity: 1
	options: 
		time: 0.2
		curve: 'linear'
...
dragArea.on Events.MouseUp, (event)->
	mask.animate('vanish')
	effect.timeUp = false
	voiceButtonText.animate('normal')
