...
emotionArea6 = new Layer
	superLayer: emotionBg
	size: emotionSizeNormal
	borderRadius: emotionSizeBig / 2
	x: emotionPadding * 6 + emotionSizeNormal * 5
	y: emotionPadding
	backgroundColor: "transparent"
...
emotion6 = new BodymovinLayer
	superLayer: emotionArea6
	jsonPath:'like1.json'
	autoplay: true
	looping: true
	width: emotionArea1.width
...
corner.states.up = 
	y: 42 * ratio - unitHeightNormal + unitHeightSmall
	options: emotionSwitchAnimation
...
emotionXpositon = () ->
	if emotionBg.states.current.name != "vanish" 
		...
		emotionArea6.x = emotionPadding * 6 + emotionArea1.width + emotionArea2.width + emotionArea3.width + emotionArea4.width + emotionArea5.width
...
emotionArea6.on "change:width", ->
	emotionXpositon()
	emotion6.subLayers[0].size = emotionArea6.size
	textLayer6.opacity = Utils.modulate(emotionArea6.width, [textStart, emotionSizeBig], [0, 1])
	textLayer6.y = Utils.modulate(emotionArea6.width, [emotionSizeNormal, emotionSizeBig], [0, -textHeight])
	textLayer6.centerX()
	...
	if unitStat == 1
		switch hold
			...
			when 4
				...
				unitStat = 0
			when 5
				emotionJumper.image = "images/love.png"
...
holderControl = ->
	if y0 <= event.point.y <= y1
		if x0 <= event.point.x < x1
			...
			holder.x = 5
		else if x5 <= event.point.x < x6
			holder.x = 6
		else
			holder.x = 0
		
		if x0 <= event.point.x <= x6
			holder.y = 1
		else 
			holder.y = 0
	else
...
x5 = mainScreen.x + emotionUnit.x + emotionPadding * 5.5 + emotionArea1.width + emotionArea2.width + emotionArea3.width + emotionArea4.width + emotionArea5.width

x6 = mainScreen.x + emotionUnit.x + emotionPadding * 7 + emotionArea1.width + emotionArea2.width + emotionArea3.width + emotionArea4.width + emotionArea5.width + emotionArea6.width
...
emotionArea6.on Events.TouchEnd, ->
	if unitStat == 1
		infoChange(6)
		active(6)
